...
@author: physiology
    ...
    ## permutation 코드 변형
    ## 열성만 permutation(열성 본초의 총 개수를 고정)
    ## 하나의 본초가 한, 열 두개의 속성을 가질 수도 있게
...
def permutation_nation(n=None):
    ...
    ## permutation 코드 변형
    ## 각 속성을 permutation(한성, 열성 등 사기의 총 개수를 고정)
    #### 하나의 본초가 한, 열 두개의 속성을 가질 수도 있게
    
    ## shuf_nation = np.random.permutation(nationdata1) 추가
    #### feature에 사용량도 permutation 추가
    shuffle1 = np.zeros((len(feature), n))
    for i in range(n):
        shuf_nation = np.random.permutation(nationdata1)
        
        for j in range(booleandata.shape[0]):
            booleandata[j,:] = np.random.permutation(booleandata[j,:])
